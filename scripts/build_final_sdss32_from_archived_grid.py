...
Purpose
-------
Reconstruct the paper-ready SDSS3.2-Final table from archived profile-grid results.
...
def collect_final_rows(root: Path, strict: bool = False) -> pd.DataFrame:
    ...
    for dataset, spec in FINAL_PROFILES.items():
        ...
        rows.append({
            ...
            "source_summary_file": str(summary_file),
        })
...
def collect_seed_detail(root: Path, final: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, final_row in final.iterrows():
        ...
        # Fallback: some releases may only have the selected-detail file for a dataset.
        # The selected-detail file still contains the 5 seed-level rows for the fixed selected profile.
        if not detail_files:
            detail_files = sorted(root.glob(f"{key}_sdss32_final_selected_detail_*.csv"), key=lambda p: p.stat().st_mtime, reverse=True)
        if not detail_files:
            print(f"[WARN] No detail or selected-detail file found for {dataset}; seed-level detail will be skipped.")
            continue
        ...
        if sub.empty:
            print(f"[WARN] No seed-level rows matched final profile for {dataset} in {detail_file}.")
            continue

        sub.insert(0, "dataset_label", dataset)
        sub.insert(1, "source_detail_file", str(detail_file))
        rows.append(sub)
...
def write_outputs(final: pd.DataFrame, seed_detail: pd.DataFrame, out_dir: Path) -> None:
    ...
    test1 = final[["dataset", "test1_f1_text"]].rename(columns={"test1_f1_text": "SDSS3.2-Final"})
    test2 = final[["dataset", "test2_f1_text"]].rename(columns={"test2_f1_text": "SDSS3.2-Final"})
    ...
    readme = f"""# Final SDSS3.2 result provenance
