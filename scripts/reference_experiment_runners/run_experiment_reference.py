...
def main():
    ...
    try:
        sampled_neg = sampler.sample(positives=pos_df, pool=neg_pool, k=n_need, dist_col="distance").reset_index(drop=True)
    except TypeError:
    ...
    bal_df = pd.concat([pos_df, sampled_neg.drop(columns=["distance"], errors="ignore")], axis=0).reset_index(drop=True)
