...
# ===================== Additional negative samplers for reviewer comparison =====================
# These samplers are defined locally so you do NOT need to replace sdsskg/sampling.py.
# They use the same interface as the original samplers.

class SemiHardSampler(RandomSampler):
    """Select semi-hard negatives: not the closest extreme hard cases, but middle-hard cases.

    The negative pool is sorted by distance ascending. Smaller distance means more similar to
    positives. Semi-hard samples are selected from the q_low-q_high segment.
    """
    def __init__(self, seed: int = 42, q_low: float = 0.20, q_high: float = 0.60):
    ...
    def sample(self, positives: pd.DataFrame, pool: pd.DataFrame, k: Optional[int] = None, dist_col: str = "distance") -> pd.DataFrame:
        ...
        lo = max(0, int(np.floor(n * self.q_low)))
        hi = min(n, int(np.ceil(n * self.q_high)))
        cand = df.iloc[lo:hi].copy()
        if len(cand) >= k:
            idx = self.rng.choice(len(cand), size=k, replace=False)
            return cand.iloc[idx].reset_index(drop=True)
        # Backfill by expanding to all negatives if the semi-hard interval is too small.
        rest = df.drop(index=range(lo, hi), errors="ignore").reset_index(drop=True)
        need = k - len(cand)
        if len(rest) > 0:
            add = rest.iloc[self.rng.choice(len(rest), size=min(need, len(rest)), replace=False)]
            cand = pd.concat([cand, add], axis=0)
        return cand.reset_index(drop=True).iloc[:k].reset_index(drop=True)


class CurriculumSampler(RandomSampler):
    """Static curriculum-style negative sampler.

    It keeps a larger easy/mid proportion than all-hard sampling, approximating an easy-to-hard
    curriculum within a single training set construction.
    """
    def __init__(self, seed: int = 42, hard_ratio: float = 0.30, mid_ratio: float = 0.40, easy_ratio: float = 0.30):
        super().__init__(seed=seed)
    ...
    def _take(self, df: pd.DataFrame, need: int) -> pd.DataFrame:
        if need <= 0 or len(df) == 0:
            return df.iloc[:0].copy()
        if len(df) <= need:
            return df.copy()
        idx = self.rng.choice(len(df), size=int(need), replace=False)
    ...
    def sample(self, positives: pd.DataFrame, pool: pd.DataFrame, k: Optional[int] = None, dist_col: str = "distance") -> pd.DataFrame:
        ...
        hard_df = df.iloc[thirds[0]].reset_index(drop=True)
        mid_df = df.iloc[thirds[1]].reset_index(drop=True)
        easy_df = df.iloc[thirds[2]].reset_index(drop=True)
        k_h = int(round(k * self.hard_ratio))
        k_m = int(round(k * self.mid_ratio))
        k_e = k - k_h - k_m
        out = pd.concat([
            self._take(hard_df, k_h),
            self._take(mid_df, k_m),
            self._take(easy_df, k_e),
        ], axis=0).reset_index(drop=True)
        if len(out) < k:
            # backfill from the hardest remaining samples
            selected_keys = set(out.astype(str).agg("||".join, axis=1).tolist()) if len(out) else set()
            rem_mask = ~df.astype(str).agg("||".join, axis=1).isin(selected_keys)
            backfill = df.loc[rem_mask].head(k - len(out))
            out = pd.concat([out, backfill], axis=0).reset_index(drop=True)
        return out.iloc[:k].sample(frac=1.0, random_state=self.seed).reset_index(drop=True)


class AdversarialScoreSampler(CurriculumSampler):
    """Model-guided adversarial negative sampler.

    It expects `dist_col` to be an adversarial score, where smaller values indicate samples that
    the warm-up model is more likely to misclassify as positive. It samples mostly from the most
    adversarial region while retaining mid/easy negatives for stability.
    """
    def __init__(self, seed: int = 42, hard_ratio: float = 0.60, mid_ratio: float = 0.30, easy_ratio: float = 0.10):
        super().__init__(seed=seed, hard_ratio=hard_ratio, mid_ratio=mid_ratio, easy_ratio=easy_ratio)
...
def _infer_and_exclude_cols(df: pd.DataFrame, label_col: str, cfg: dict) -> List[str]:
    ...
    for c in df.columns:
        ...
        if sc == label_col or "标签" in sc or "label" in scl:
            out.append(c)
            continue
        if "疾病" in sc or "diagnosis" in scl or "disease" in scl:
            out.append(c)
            continue
...
def _compute_model_guided_adversarial_scores(
    train_df: pd.DataFrame,
    y_all: np.ndarray,
    text_all_emb: np.ndarray,
    ...
    """Warm-up logistic model to score candidate negative samples.

    The score is computed using training data only. External test sets are never used.
    Returned score is 1 - P(positive), so smaller score means more adversarial.
    """
    from sklearn.linear_model import LogisticRegression

    rng = np.random.default_rng(int(seed))
    X_num_df = _build_numeric_X(train_df, feature_cols)
    fills = _fit_imputer(X_num_df, strategy="median")
    X_num_df = _apply_imputer(X_num_df, fills)
    means, stds = _zscore_fit(X_num_df)
    X_num_df = _zscore_apply(X_num_df, means, stds)
    X_all = np.concatenate([
        X_num_df.to_numpy(dtype=np.float32),
        text_all_emb.astype(np.float32),
    ], axis=1)
    X_all = _ensure_finite_np(X_all)

    pos_idx = np.where(pos_mask)[0]
    neg_idx = np.where(neg_mask)[0]
    n_neg_need = max(1, int(np.ceil(len(pos_idx) * float(neg_per_pos))))
    n_neg_need = min(n_neg_need, len(neg_idx))
    warm_neg_idx = rng.choice(neg_idx, size=n_neg_need, replace=False)
    warm_idx = np.concatenate([pos_idx, warm_neg_idx])
    ...
    clf.fit(X_all[warm_idx], y_all[warm_idx])
    p_pos = clf.predict_proba(X_all[neg_idx])[:, 1].astype(np.float32)
    adv_score = (1.0 - p_pos).astype(np.float32)

    info = {
        "mode": "model_guided_adversarial_sampling",
        "warmup_model": "LogisticRegression(class_weight='balanced')",
        "warmup_uses_external_test": False,
        "n_pos_warmup": int(len(pos_idx)),
        "n_neg_warmup": int(n_neg_need),
        "p_positive_mean_for_neg_pool": float(np.mean(p_pos)),
        "p_positive_std_for_neg_pool": float(np.std(p_pos)),
        "p_positive_q90_for_neg_pool": float(np.quantile(p_pos, 0.90)),
        "adversarial_score_definition": "1 - predicted_positive_probability; smaller means more adversarial",
    }
    return adv_score, info
...
def _pick_sampler(cfg: dict):
    ...
    if sampler_name in {"semi_hard", "semihard", "semi-hard", "semi_hard_ns"}:
        q_low = float(cfg.get("sampler", {}).get("q_low", 0.20))
        q_high = float(cfg.get("sampler", {}).get("q_high", 0.60))
        return SemiHardSampler(seed=seed, q_low=q_low, q_high=q_high), neg_per_pos
    if sampler_name in {"curriculum", "curriculum_ns", "curriculum_sampling"}:
        hard_ratio = float(cfg.get("sampler", {}).get("hard_ratio", 0.30))
        mid_ratio = float(cfg.get("sampler", {}).get("mid_ratio", 0.40))
        easy_ratio = float(cfg.get("sampler", {}).get("easy_ratio", 0.30))
        return CurriculumSampler(seed=seed, hard_ratio=hard_ratio, mid_ratio=mid_ratio, easy_ratio=easy_ratio), neg_per_pos
    if sampler_name in {"adversarial", "adversarial_ns", "adv", "model_guided_adversarial"}:
        hard_ratio = float(cfg.get("sampler", {}).get("hard_ratio", 0.60))
        mid_ratio = float(cfg.get("sampler", {}).get("mid_ratio", 0.30))
        easy_ratio = float(cfg.get("sampler", {}).get("easy_ratio", 0.10))
        return AdversarialScoreSampler(seed=seed, hard_ratio=hard_ratio, mid_ratio=mid_ratio, easy_ratio=easy_ratio), neg_per_pos
...
def main():
    ...
    sampler_name = str(cfg.get("sampler", {}).get("name", "sdss")).lower()
    sampler_dist_col = "distance"

    # Model-guided adversarial sampling: score only the training negative pool.
    # External test sets are never used to construct the adversarial scores.
    if sampler_name in {"adversarial", "adversarial_ns", "adv", "model_guided_adversarial"}:
        adv_scores, adv_info = _compute_model_guided_adversarial_scores(
            train_df=train_df,
            y_all=y_all,
            text_all_emb=text_all_emb,
        ...
        neg_pool["adversarial_score"] = adv_scores
        sampler_dist_col = "adversarial_score"
        save_json(adv_info, out_dir / "adversarial_sampling_info.json")
        neg_pool[["distance", "adversarial_score"]].to_csv(
            out_dir / "negative_pool_adversarial_scores.csv",
            index=False,
            encoding="utf-8-sig",
        )
    ...
    try:
        sampled_neg = sampler.sample(positives=pos_df, pool=neg_pool, k=n_need, dist_col=sampler_dist_col).reset_index(drop=True)
    except TypeError:
    ...
    bal_df = pd.concat([pos_df, sampled_neg.drop(columns=["distance", "adversarial_score"], errors="ignore")], axis=0).reset_index(drop=True)
